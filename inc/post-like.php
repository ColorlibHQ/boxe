 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_boxe_process_simple_like', 'boxe_process_simple_like' );
add_action( 'wp_ajax_boxe_process_simple_like', 'boxe_process_simple_like' );
function boxe_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'boxe' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'boxe' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'boxe' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'boxe' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'boxe' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'boxe' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'boxe_show_user_likes' );
add_action( 'edit_user_profile', 'boxe_show_user_likes' );
function boxe_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'boxe' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'boxe' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
