...
if ( ! function_exists( 'boxe_breadcrumbs' ) ) {
    function boxe_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'boxe' )
        );
        $args      = apply_filters( 'boxe_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html .= '<li class="breadcrumb-item"><a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'boxe' ).' </a></li>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<li class="breadcrumb-item active" aria-current="page">'.esc_html__('Search results for:','boxe') . get_search_query() . '</li>';
        } elseif ( is_404() ) {
            $html .= '<li class="breadcrumb-item active" aria-current="page">' . esc_html__( 'Error 404', 'boxe' ) . '</li>';
        } elseif ( is_home() ) {
            $html .= '<li class="breadcrumb-item active" aria-current="page">' . get_the_title( get_option( 'page_for_posts' ) ) . '</li>';
        }
        $html .= '</ol>';
        $html .= '</nav>';
        $html = apply_filters( 'boxe_breadcrumbs_filter', $html );
