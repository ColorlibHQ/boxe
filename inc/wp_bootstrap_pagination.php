...
function boxe_pagination( $args = array() ) {
    ...
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'boxe_pagination_defaults', $defaults )
    );
    ...
    if ( $previous && (1 != $page) )
        $echo .= '<li class="previous pgn__prev"><a href="' . esc_attr( $previous ) . '" title="' . esc_html__( 'previous', 'boxe' ) . '">' . esc_html__( 'Previous', 'boxe' ) . '</a></li>';
    ...
    if ($next && ($count != $page) )
        $echo .= '<li class="next pgn__next"><a href="' . esc_attr( $next ) . '" title="' . esc_html__( 'next', 'boxe') . '">' . esc_html__( 'Next', 'boxe') . '</a></li>';
